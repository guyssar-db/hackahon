...
import { Bars3Icon, XMarkIcon, HomeIcon, UsersIcon, CalendarIcon, ChartPieIcon, DocumentDuplicateIcon, FolderIcon,CogIcon } from '@heroicons/react/24/outline'
...
const navigation = [
  ...
  { name: 'Event Performance', href: '#', icon: ChartPieIcon, current: false },
  { name: 'Trickets', href: '#', icon: DocumentDuplicateIcon, current: false },
          ...
          <DialogPanel className="relative mr-16 flex w-full max-w-xs flex-1 transform transition duration-300 ease-in-out">
            ...
            <div className="relative flex grow flex-col gap-y-5 overflow-y-auto bg-gray-900 px-6 pb-2">
              <nav className="flex flex-1 flex-col">
                <ul role="list" className="flex flex-1 flex-col gap-y-3 pt-20">
                  {navigation.map((item) => (
                    <li key={item.name}>
                      <a
                        href={item.href}
