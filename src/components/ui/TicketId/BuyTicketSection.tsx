    ...
    <div id="buy-ticket" className="space-y-4 bg-white dark:bg-gray-700 shadow-md rounded-lg px-4 py-6">
      ...
      <div className="flex flex-wrap gap-3">
        {Object.entries(pricing).map(([key, value]) => {
          ...
          return (
            <button key={key} onClick={() => setSelectedPrice(key)}
      ...
      <button onClick={handlePurchase} className="px-6 py-3 bg-green-500 text-white rounded-lg hover:bg-green-600 cursor-pointer">
        สั่งซื้อ
      </button>
    </div>
  );
