...
export interface Event {
    ...
    category: string;
    status: string;
    organizer: { name: string; contact: string; phone: string };
    schedule?: {
        startDate: string;
        endDate: string;
        startTime: string;
    ...
    pricing?: { currency?: string; [key: string]: number | string | undefined };
    capacity?: { max?: number; registered?: number };
    images?: { banner?: string; thumbnail?: string; gallery?: string[] };
    tags?: string[];
    requirements?: string[];
    includes?: string[];
    tracks?: string[];
...
export default function EventList({ initialEvents }: EventListProps) {
    ...
    const filteredEvents = events
        .filter((e) => e.schedule && e.location && e.pricing)
        .filter((e) => (category ? e.category === category : true))
        .filter((e) => {
            if (!priceRange || !e.pricing) return true;
            const [min, max] = priceRange.split('-').map(Number);
            const prices = Object.values(e.pricing).filter(
                    ...
                    {filteredEvents.map((event) => {
                        ...
                        const priceText =
                            min === max
                                ...
                                : `${min.toLocaleString()} - ${max.toLocaleString()} ${event.pricing?.currency || ''}`;

                        const hasImage =
                            event.images?.thumbnail &&
                            event.images.thumbnail.trim() !== '';
                            ...
                            >
                                ...
                                <div className="p-4 dark:text-white">
                                    <p className="text-xs text-gray-500 dark:text-gray-400 mb-2 text-center">
                                        {event.schedule
                                            ? `${new Date(event.schedule.startDate).toLocaleDateString('th-TH', { day: '2-digit', month: 'short' })} - ${new Date(event.schedule.endDate).toLocaleDateString('th-TH', { day: '2-digit', month: 'short' })}`
                                            : 'วันที่ไม่ระบุ'}
                                    </p>
